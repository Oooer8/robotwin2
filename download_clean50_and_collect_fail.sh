...
prepare_one() {
  ...
  # 网络易抖动: 自动重试 + 端点回退 (hf-mirror <-> huggingface.co)
  if [ ! -f "$zip_path" ]; then
    echo -e "${YELLOW}[DL]${NC}    ${task}/${CONFIG_ZIP}"

    local endpoints=("$HF_ENDPOINT")
    ...
    for ep in "${endpoints[@]}"; do
      for attempt in 1 2 3; do
        if HF_ENDPOINT="$ep" HF_HUB_DOWNLOAD_TIMEOUT=60 \
            "$HF_CLI_BIN" download "$REPO" "$zip_rel" --repo-type dataset \
            --local-dir "$RAW_ROOT"; then
          break 2
        fi
        echo -e "${YELLOW}[RETRY]${NC} ${task}: 端点 ${ep} 第 ${attempt}/3 次失败, 稍后重试..."
        sleep $((attempt * 4))
      done
    done
  fi
  if [ ! -f "$zip_path" ]; then
    echo -e "${RED}[FAIL]${NC}  ${task}: 多次重试后仍未下载到 ${zip_path}"
    echo -e "         排查: 1) 网络/代理是否可达 Hub; 2) 换端点 HF_ENDPOINT=https://huggingface.co;"
    echo -e "               3) 降低并行 MAX_JOBS=2; 4) 手动测试 '$HF_CLI_BIN download $REPO $zip_rel --repo-type dataset --local-dir $RAW_ROOT'"
    return 1
  fi
